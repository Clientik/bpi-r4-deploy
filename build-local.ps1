...
echo ""
echo "=== [2/5] Preparing repo-cache ==="
mkdir -p "\$CACHE_DIR"

if [ ! -f "\$CACHE_DIR/openwrt.tar.gz" ]; then
  echo "  Cloning OpenWrt (once)..."
  ...
  echo "  OpenWrt: cache found, skipping."
fi

if [ ! -f "\$CACHE_DIR/mtk-openwrt-feeds.tar.gz" ]; then
  echo "  Cloning MTK feeds (once)..."
  ...
  rm -rf /tmp/mtk-clone
  echo "  MTK feeds cached."
else
  echo "  MTK feeds: cache found, skipping."
fi
