...
| **mwan3 WAN failover** | `my_files/etc-files/config/mwan3` (shipped as `/etc/config/mwan3`); defconfig `mwan3` + `luci-app-mwan3` | Wired `wan` = metric 1 (primary), `FB350` = metric 2 (backup). track_ip `1.1.1.1`/`8.8.8.8`, fail after 3×5 s → fail over to the 5G modem, back when wired recovers. Static file overrides the mwan3 package's example config. FB350 tracking is loosened for mobile (count 3, timeout 5, interval 10, down 5, up 2). |
| **fwupdate — OTA from this fork's GitHub** (self-contained package, easy to remove) | `my_files/fwupdate/**` (CLI `/usr/sbin/fw-update` + `/etc/config/fwupdate`), `my_files/luci-app-fwupdate/**` (LuCI page System → Firmware Update); defconfig `fwupdate` + `luci-app-fwupdate`; a `# ===== fwupdate OTA (REMOVABLE) =====` block in the two builders that vendors the packages and stamps `/etc/fork-release` (repo/profile/commit/date) | Checks the fork's GitHub releases for the running variant (auto by RAM + build profile + PoE; override via `uci fwupdate.main.tag`, or the LuCI variant), compares release date to the build stamp, and `sysupgrade`s the matching `bpi-r4.itb` (keeps settings). NOT the official Attended Sysupgrade. **To remove entirely:** delete `my_files/fwupdate` + `my_files/luci-app-fwupdate`, the two `CONFIG_PACKAGE_fwupdate*` defconfig lines, and the `fwupdate OTA` block in both builders — nothing else depends on it. Compiles no code (pure sh/JS) but needs a build to package + a run to validate. |
